...
public class MurderOfCrows {
	...
	private void findTheDiamond() {
		boolean diamondMissing = true;
		int deadCrows = 0;
		String guiltyCrow = "not found";
		...
		for (Crow c : theMurder) {
			ArrayList<String> Contents = c.getStomachContents();
			if (diamondMissing) {
				deadCrows++;
			}
			for (String s : Contents) {
				if (s.equals("diamond")) {
					guiltyCrow = c.getName();
					diamondMissing = false;
				}
			}
		}
		System.out.println("The guilty crow was " + guiltyCrow + ".");
		// 2. How many innocent crows had to die before the diamond was found?
		if (deadCrows == 1) {
			System.out.println(deadCrows + " crow was killed before the diamond was found.");
		} else {
			System.out.println(deadCrows + " crows were killed before the diamond was found.");
		}
	}
