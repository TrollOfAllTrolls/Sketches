...
public class HospitalTest extends TestCase {
	...
	public void testAssignDoctor() throws Exception {
		...
		assertEquals(1, testDoctor.getPatients().length);
		testDoctor.assignPatient(new Patient());
		assertEquals(2, testDoctor.getPatients().length);
		testDoctor.assignPatient(new Patient());
		assertEquals(3, testDoctor.getPatients().length);
	}
