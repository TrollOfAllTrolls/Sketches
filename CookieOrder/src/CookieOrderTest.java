...
/* These unit tests will help you check your answers to the 2010 AP question on ArrayLists. 
 * You should try answering the question on paper first. Cookie Order is the first question on the paper.
...
public class CookieOrderTest {
	 ...
	 * PART A
	 * 
	 * The getTotalBoxes method computes and returns the sum of the number of boxes of all cookie orders. If there are
	 * no cookie orders in the master order, the method returns 0.
	 */
	@Test
	 ...
	 * PART B
	 * 
	 * The removeVariety method updates the master order by removing all of the cookie orders in which the variety of
	 * cookie matches the parameter cookieVar. The master order may contain zero or more cookie orders with the same
	 * variety as cookieVar. The method returns the total number of boxes removed from the master order.
	 */
	@Test
